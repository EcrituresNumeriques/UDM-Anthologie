...
module.exports = {
  ...
  callback: function * () {
    ...
    if (provider === 'local') {
      ...
      try {
        var user;
        yield User.forge().fetch(query).then(function(data){
          user = data;
        });
        ...
        ctx.status = 200;
        ctx.body = { jwt: strapi.api.user.services.jwt.issue(user), user: user };
  ...
  forgotPassword: function * () {
    ...
    try {
      ...
      yield User.forge().fetch({email: email}).then(function(data){user = data});
    ...
    // Update the user.
    try {
      yield User.forge({"id": user.attributes.id}).save(user.changed).then(function(data){
        console.log(data);
      });
    } catch (err) {
      console.log(err);
  ...
  changePassword: function * () {
    ...
    if (params.password && params.passwordConfirmation && params.password === params.passwordConfirmation && params.code) {
      ...
      try {
        yield User.forge({resetPasswordToken: params.code}).fetch().then(function(data){
          user = data;
        });
